...
vtx_stack* create_vtx_stack(){
	...
	vs->vtcs = malloc(MAX_STACK_SIZE * sizeof(vtx*));
	//TODO: Uninitialized
	vs->size = 0;
	return vs;
}
...
int push_vtx_stack(vtx_stack* vs, vtx* v){
...
vtx* pop_vtx_stack(vtx_stack* vs){
	if (vs->size == 0)
		return NULL;
	vs->size--;
...
vtx* postfix_to_virtex(stack* postfix){
	...
	while (reverse->size > 0){
		...
		if (!token_is_operator(curr) && ! token_is_escaped(curr)){
			vtx* temp = vtx_create(VT_LITERAL);
			...
			vtx* second = pop_vtx_stack(arg_stack);
	...
	vtx* ans = pop_vtx_stack(arg_stack);
...
void parse_file(char* file_name){
	...
	printf("postfix\n");
	print_stack(postfix);
	vtx* result = postfix_to_virtex(postfix);
	vtx_print(result);
